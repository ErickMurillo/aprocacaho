...
def dashboard(request,template="productores/dashboard.html"):
	...
	for year in request.session['year']:
		...
		#rendimiento
		#areas certificadas
		area_cert = filtro.filter(year = year,plantacion__edad__in = [3,4,5],certificacion__cacao_certificado = '1').aggregate(
									area_cacao = Sum('plantacion__area'))['area_cacao']
		if area_cert == None:
			area_cert = 0

		#areas no certificadas
		area_no_cert = filtro.filter(year = year,plantacion__edad__in = [3,4,5],certificacion__cacao_certificado = '2').aggregate(
										area_cacao = Sum('plantacion__area'))['area_cacao']
